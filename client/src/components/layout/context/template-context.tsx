...
export function factoryContext(): TemplateContextValue {
	...
	const actions = useMemo<Omit<TemplateContextValue, 'templates'>>(
		() => ({
			/**
			 * Регистрирует шаблон в указанном слоте.
			 *
			 * @param {string} slotName - Имя слота, в который нужно зарегистрировать шаблон.
			 * @param {React.ReactNode} element - React-элемент, который будет связан с этим слотом.
			 *
			 * @example
			...
			/**
			 * Удаляет шаблон из указанного слота.
			 *
			 * @param {string} slotName - Имя слота, из которого нужно удалить шаблон.
			 *
			 * @example
			 * unregister('header');
			 */
